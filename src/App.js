...
function App() {
  ...
  const handleGenerate = async () => {
    const res = await fetch('https://visionbuilder-backend-1.onrender.com/api/generate', {
      method: 'POST',
  ...
  const handleUpload = async (e) => {
    ...
    const res = await fetch('https://visionbuilder-backend-1.onrender.com/api/upload', {
      method: 'POST',
      body: formData
    });
    const data = await res.json();
